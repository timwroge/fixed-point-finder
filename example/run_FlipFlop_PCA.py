...
sys.path.insert(0, '/home/matt/gcloud/gold/code/fixed-points/')
import fp_plot
...
state_trajs = example_predictions['state']
if is_lstm:
 	state_traj_bxtxd = tf_utils.convert_from_LSTMStateTuple(state_trajs)
else:
 	state_traj_bxtxd = state_trajs
...
initial_states = fpf.sample_states(state_trajs,
								   n_inits=N_INITS,
								   rng=ff.rng,
...
ax = fp_plot.fig_setup()
fp_plot.plot_trajectories(ax, state_traj_bxtxd,
	pca=pca,
	plot_batch_idx=range(30),
	plot_start_time=10)

fp_plot.plot_fps(ax, unique_fps,
	pca=None,
